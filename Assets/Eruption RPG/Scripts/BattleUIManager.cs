...
public class BattleUIManager : MonoBehaviour
{
    ...
    {
        playerHP.text = $"{playerStatus.GetCurrentHealth().ToString()} / {playerStatus.GetMaxHealth().ToString()}";
    ...
    public void MonsterHPUpdater(Monster encounterMonster)
    {
        ...
        {
            int totalBars = Mathf.CeilToInt((float)maxHP / hpPerBar); // 체력이 몇 줄인지 (체력이 52000이라면 6줄) // 지금은 안쓰는데 애니메이션 만들 때 사용 가능
